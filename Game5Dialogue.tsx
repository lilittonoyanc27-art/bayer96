...
const BANK_WORDS = [
  ...
  { es: 'cuesta', hy: 'արժե' },
  { es: 'tarjeta', hy: 'քարտ' },
  { es: 'gracias', hy: 'շնորհակալություն' }
];
...
const DIALOG_LINES: DialogLine[] = [
  {
    id: 1,
    speaker: 'gayane',
    textTemplate: 'Hola, quiero {gap} jamón español y pan fresco.',
    gapId: 'comprar',
    correctWord: 'comprar',
    translationHy: 'Ողջույն, ես ուզում եմ գնել իսպանական խամոն և թարմ հաց:',
  },
  {
    id: 2,
    speaker: 'cajero',
    textTemplate: '¡Hola! Muy bien. ¿Necesita una {gap} para llevar las cosas?',
    gapId: 'bolsa',
    correctWord: 'bolsa',
    translationHy: 'Ողջույն: Շատ լավ: Տոպրակ ձեզ պե՞տք է իրերը տանելու համար:',
  },
  {
    id: 3,
    speaker: 'gor',
    textTemplate: 'Sí, por favor. ¿Y cuánto {gap} este queso?',
    gapId: 'cuesta',
    correctWord: 'cuesta',
    translationHy: 'Այո, խնդրում եմ: Իսկ ինչքա՞ն արժե այս պանիրը:',
  },
  {
    id: 4,
    speaker: 'cajero',
    textTemplate: 'Cuesta cinco euros. ¿Desea pagar con {gap} o en efectivo?',
    gapId: 'tarjeta',
    correctWord: 'tarjeta',
    translationHy: 'Այն արժե հինգ եվրո: Ցանկանո՞ւմ եք վճարել քարտով, թե՞ կանխիկ:',
  },
  {
    id: 5,
    speaker: 'gor',
    textTemplate: 'Con tarjeta, por favor. ¡Muchas {gap}!',
    gapId: 'gracias',
    correctWord: 'gracias',
    translationHy: 'Քարտով, խնդրում եմ: Շատ շնորհակալություն:',
  }
];

export default function Game5Dialogue({
  activePlayerName,
  onNextStage,
  awardPoints,
  activePlayerKey,
    ...
    try {
      if ('speechSynthesis' in window) {
        const fullText = DIALOG_LINES.map(line => {
          ...
          const speakerName = line.speaker === 'gor' ? 'Gor' : line.speaker === 'gayane' ? 'Gayane' : 'Cajero';
          return `${speakerName}: ${line.textTemplate.replace('{gap}', word)}`;
        }).join('. ');
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 font-sans">
      ...
      <div className="bg-white border-2 border-slate-200 rounded-2xl p-6 mb-8 shadow-sm text-base text-slate-700">
        <p className="font-display font-black text-slate-800 text-lg mb-2 uppercase tracking-wide">
          Գոռն ու Գայանեն գնումներ են կատարում իսպանական սուպերմարկետում և վճարում են դրամարկղի մոտ:
        </p>
      ...
      <div className="bg-slate-50 border-2 border-slate-200 rounded-3xl p-5 md:p-8 mb-8 space-y-6 shadow-inner">
        {DIALOG_LINES.map((line) => {
          ...
          const isGayane = line.speaker === 'gayane';
          ...
          let avatarEmoji = '👩‍💼';
          let avatarStyle = 'bg-slate-100 border-2 border-slate-300';
          let bubbleAlign = 'ml-auto mr-auto';
          let speakerLabel = 'Cajero • Գանձապահ';

          if (isGor) {
            avatarEmoji = '👦';
            avatarStyle = 'bg-blue-50 border-2 border-blue-200';
            bubbleAlign = 'mr-auto';
            speakerLabel = 'Gor • Գոռ';
          } else if (isGayane) {
            avatarEmoji = '👧';
            avatarStyle = 'bg-pink-50 border-2 border-pink-200';
            bubbleAlign = 'mr-auto';
            speakerLabel = 'Gayane • Գայանե';
          }

          return (
            <div
              key={line.id}
              className={`flex items-start gap-4 max-w-2xl ${bubbleAlign}`}
            >
              {/* Avatar */}
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 shadow-md ${avatarStyle}`}>
                {avatarEmoji}
              </div>

              {/* Chat Bubble */}
              <div className="rounded-3xl p-5 border-2 shadow-md relative bg-white border-slate-100 text-left max-w-full flex-grow">
                {/* Speaker tag */}
                <div className={`text-xs font-display font-black uppercase mb-1.5 ${
                  isGor ? 'text-blue-500' : isGayane ? 'text-pink-500' : 'text-slate-500'
                }`}>
                  {speakerLabel}
                </div>
      ...
      <div className="flex justify-between items-center mt-8">
        ...
        {allGapsFilled && !showResults ? (
          ...
          >
            Անցնել Վերջնական Վիկտորինային <ArrowRight className="w-4 h-4" />
          </button>
        ) : null}
      </div>
    </div>
